...
# Installation #
Please see [Installation](Installation) for install instructions.

# Quick Reference #

Please see the [Overview](Overview) page for a list of Special Measure functions,
and [Error Messages](Error Messages) for a list of common error messages and what to do.

# Configuration Data #
  * [scan](Scans) (structure)
  * [smdata](smdata) (global structure)
...
To add a channel, use the [smaddchannel](smaddchannel) function. Note that depending
...
To remove one or several channels ch, just type "[smdata](smdata#channels).channels(ch) = `[]`;"
...
The measurement task to be excecuted by [smrun](smrun) is defined by a [scan](Scans_new) struct passed to [smrun](smrun). For explanations of its fields, see [scan](Scans_new).
